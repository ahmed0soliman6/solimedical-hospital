...
سيُستخدم Firestore كمصدر بيانات رئيسي بعد مرحلة تشغيل مزدوج واختبار. لن تتصل الواجهة مباشرة بقاعدتين بشكل متنافس؛ ستستخدم طبقة بيانات واحدة، ويُحدد المصدر من خلال إعداد واحد. تبقى Supabase للقراءة الاحتياطية أثناء الترحيل فقط.
...
3. إضافة وضع `firebase-shadow` لكتابة نسخة اختبارية إلى Firestore مع إبقاء Supabase مصدراً للواجهة.
4. مقارنة عدد السجلات ونتائج التقارير بين المصدرين.
5. نقل البيانات التاريخية باستخدام Firebase Admin على خادم آمن، وليس من مفتاح الواجهة.
6. تحويل القراءة إلى Firestore بعد نجاح الاختبارات.
7. تحويل الكتابة إلى Firestore، مع إبقاء تصدير Supabase احتياطياً لفترة.
8. نقل المستخدمين إلى Firebase Auth وإجبار إعادة تعيين كلمات المرور عند الحاجة.
9. تعطيل Supabase فقط بعد التحقق من المرضى والزيارات والوارد والمنصرف والحذف والتقارير.

## ملاحظة تشغيلية

لا يمكن تنفيذ الترحيل الكامل من داخل ملف HTML وحده بأمان؛ إعداد Firebase client يمكن أن يكون في الواجهة، لكن نقل البيانات وإنشاء المستخدمين وتحديث التقارير يتطلبان بيئة خادم أو Cloud Functions ومفاتيح إدارية لا يجوز وضعها في المستودع أو داخل Electron.
